...
export default {
  ...
  methods: {
    ...
    async getImageResults(){
        ...
        await fetch(
        `https://www.googleapis.com/customsearch/v1?q=${carResult}&key=${googleApiKey}&cx=001912728085642910227:akqkacx3uim&searchType=image&fileType=jpg&imgSize=small&alt=json`
      )
        .then(response => response.json())
