<template>
   ...
   <div v-for="result in results" v-bind:key="result">
       <p>{{v-bind:src="result.links"))</p>
       <p v-bind:src="result.links"/>    </div>
    </div>
    </div>
        ...
        methods: {
           async getResult(){
            ...
            'Access-Control-Allow-Origin':'*',
              ...
              alert(JSON.stringify(myJson.collection.items));
              this.results = JSON.stringify(myJson.collection.items);
            }
        }
    }
</script>
