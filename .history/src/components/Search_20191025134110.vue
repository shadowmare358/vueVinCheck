...
export default {
  ...
  methods: {
    ...
    async getImageResults(query){
        const cxKey =
       await fetch(
        `https://www.googleapis.com/customsearch/v1?q=honda_jazz&key=AIzaSyCAJ2rKXL-X_daAy2dveCO--E5ciLgIK6k&cx=&searchType=image&fileType=jpg&imgSize=small&alt=json`
      )
        .then(response => response.json())
        .then(myJson => alert(myJson))
        .catch(err => alert(`Cannot get requested data ${err}`));
