  ...
  methods: {
    async getResults() {
      await fetch(
        ...
        .then(myJson => (this.results = myJson))
        .catch(err => alert(`Cannot get requested data ${err}`));
    }
    await getImageResults(){
        let https://www.googleapis.com/customsearch/v1?q=honda_jazz&key=AIzaSyCAJ2rKXL-X_daAy2dveCO--E5ciLgIK6k&cx=001912728085642910227:akqkacx3uim&searchType=image&fileType=jpg&imgSize=small&alt=json
    }
  }
};
</script>
