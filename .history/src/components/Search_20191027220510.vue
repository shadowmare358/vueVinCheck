<template>
  <div class="search">
    ...
    <input v-model="vin" v-on:keyup.enter="getVIN(vin)">
    ...
    <ul>
        ...
        <img v-for="item in vinResults" v-bind:key="item.id" :src="item.image.thumbnailLink"/>
        <li></li>
    </ul>
...
export default {
  ...
  async getVIN(vinNumber){
    const marketApiKey = "NuJ2q06UO5TRrFU2QfhteMzvIPRz5sNu"
     await fetch(
        `http://api.marketcheck.com/v1/vin/${vinNumber}/specs?api_key=${marketApiKey}`,
        {
          mode: "cors",
          headers: {
            host: "marketcheck-prod.apigee.net",
