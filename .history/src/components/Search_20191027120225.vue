<template>
  <div class="search">
    ...
    <ul>
        <li v-for="image in imagesResults" v-bind:key="image.id">{{image.url}}</li>
    </ul>
...
export default {
  ...
  methods: {
    async getResults() {
      ...
      await fetch(
        `https://marketcheck-prod.apigee.net/v1/popuar/cars?api_key=${marketApiKey}&car_type=new`,
        {
          mode: "cors",
          headers: {
    ...
    async getImageResults(){
       ...
       await fetch(
        `https://www.googleapis.com/customsearch/v1?q=honda_jazz&key=${googleApiKey}&cx=${cxKey}&searchType=image&fileType=jpg&imgSize=small&alt=json`,{
              headers: {
            "Access-Control-Allow-Origin": "*",
            "Content-Type": "multipart/form-data",
            "Access-Control-Allow-Credentials": true
